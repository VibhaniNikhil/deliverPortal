...
namespace SlimJim.Infrastructure
{
	public class VisualStudioIntegration
	{
		...
		public static void OpenSolution(string solutionPath, SlnGenerationOptions options)
		{
			var devenvPath = options.ToolPath ?? FindDevEnv(options.VisualStudioVersion);

			if (devenvPath == null || !File.Exists(devenvPath))
			{
				Log.ErrorFormat("Unable to locate Visual Studio {0} install directory.", options.VisualStudioVersion.Year);
				return;
			}
